describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i=0; i<prodItemsData.length; i++){
      ...
      nextValue = prodItemsData[i];
      if (nextValue.title.length == 0) { allArePopulated = false; }
      if (nextValue.price.length == 0) { allArePopulated = false; }
      if (nextValue.image.length == 0) { allArePopulated = false; }
    }
    // Expect allArePopulated to still be true
    expect(allArePopulated).toBe(true);
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const ATCbutton = await shadowRoot.$('button');
    await ATCbutton.click();
    // Once you have the innerText property, use innerText.jsonValue() to get the text value of it
    const buttonText = await ATCbutton.evaluate(node => node.innerText);
    expect(buttonText).toContain('Remove from Cart');
  }, 2500);
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for (let i = 0; i < productItems.length; i++) {
      const shadow = await productItems[i].evaluateHandle(item => item.shadowRoot);
  
      const RFCbutton = await shadow.$('button');
  
      await RFCbutton.click();
    }
